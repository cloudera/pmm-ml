...
def explainid(N):
    ...
    uid = customer_data.pop("uid")
    customer_data.pop("index")
    customer_data.pop("Champion probability")
    data = em.cast_dct(customer_data)
    probability, explanation = em.explain_dct(data)
    
    # add back uid for display purposes
    uid_data = {'uid': uid}
    uid_data.update(data)
    
    # do not display features that were not used
    display_features = ['uid'] + em.features
    display_data = dict((k, uid_data[k]) for k in display_features)
    return {
        "data": display_data,
...
def sample_table():
    sample_ids = em.data.index.to_list()
    sample_table = []
    for ids in sample_ids:
        explanation = explainid(str(ids))
        keys = ['data', 'explanation']
        for k in keys:
            for i in explanation[k].keys():
                if i != 'uid':
                  explanation[k][i] = round(explanation[k][i], 4)
        sample_table.append(explanation)
