...
class ExplainedModel:
    ...
    def explain_df(self, df):
        ...
        e = self.explainer.explain_instance(X[0], self.pipeline.predict_proba, num_samples=1000, num_features=5).as_map()[1]
    ...
    def non_categorical_features(self):
        return list(
            self.data.select_dtypes(exclude=["category", "O"]).columns.drop(
    ...
    def stats(self):
        ...
        if not hasattr(self, "_stats"):
            self._stats = {
                c: describe(self.data[c]) for c in self.features
            }
        return self._stats
